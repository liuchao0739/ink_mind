...
/// 八一中文网数据源实现
class BaYiBookDataSource implements NovelBookDataSource {
  BaYiBookDataSource({ApiClient? apiClient}) : _apiClient = apiClient ?? ApiClient(baseUrl: _baseUrl);

  static const String _baseUrl = 'https://www.81zw.net';
  ...
  Future<List<Book>> searchRemote(String keyword) async {
    ...
    try {
      print('BaYi: Searching for $keyword');
      final response = await _apiClient.getTextFromUrl('$_baseUrl/search.php?keyword=${Uri.encodeComponent(keyword)}');
      print('BaYi: Received response length: ${response.length}');
      final books = _parseSearchResults(response);
      print('BaYi: Found ${books.length} books');
      return books;
    } catch (e) {
      print('BaYi: Search error: $e');
      // 忽略错误，返回空列表
      return const [];
    }
  }
    ...
    for (final match in matches) {
      if (match.groupCount >= 2) {
        ...
        chapters.add(Chapter(
          id: 'bayi_chapter_${path.replaceAll('/', '')}',
          bookId: 'bayi_${novelId.replaceAll('/', '')}',
          index: index,
          title: chapterTitle,
          content: '', // 内容将在需要时获取
        ));
